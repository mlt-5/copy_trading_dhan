...
class OrderManager:
    ...
    def replicate_order(self, leader_order_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            # ✅ PATCH-002: Proper productType-based detection (per DhanHQ v2 Orders API)
            # CO and BO are productType values, not separate categories
            ...
            # Check productType to determine if it's a BO or CO order
            if product_type == 'BO':
                # Bracket Order - extract BO-specific parameters
                bo_profit_value = leader_order_data.get('boProfitValue')
                bo_stop_loss_value = leader_order_data.get('boStopLossValue')
                bo_order_type = leader_order_data.get('boOrderType', 'LIMIT')
                logger.info(f"Bracket Order detected: product_type=BO, profit={bo_profit_value}, SL={bo_stop_loss_value}")
            
            elif product_type == 'CO':
                # Cover Order - extract CO-specific parameters
                # Note: CO parameters may vary, verify from super-order docs
                co_stop_loss_value = leader_order_data.get('coStopLossValue') or leader_order_data.get('stopLossValue')
                co_trigger_price = leader_order_data.get('coTriggerPrice') or leader_order_data.get('triggerPrice')
    ...
    def _place_follower_order(
        ...
        try:
            ...
            # ✅ PATCH-002: Build API params with productType properly set
            api_params = {
                'security_id': security_id,
                'exchange_segment': exchange_segment,
                'transaction_type': transaction_type,
                'quantity': quantity,
                'order_type': order_type,
                'product_type': product_type,  # ✅ CRITICAL: productType determines BO/CO
                'price': price if price > 0 else 0,
                'validity': validity
            }
            
            # Include trigger price for SL orders
            if trigger_price and order_type in ('SL', 'SL-M'):
                api_params['trigger_price'] = trigger_price
            
            # Include disclosed quantity if specified
            if disclosed_qty:
                api_params['disclosed_quantity'] = disclosed_qty
            
            # ✅ PATCH-002: Include CO parameters only if productType == 'CO'
            if product_type == 'CO' and co_stop_loss_value is not None:
                api_params['coStopLossValue'] = co_stop_loss_value
                if co_trigger_price is not None:
                    api_params['coTriggerPrice'] = co_trigger_price
            
            # ✅ PATCH-002: Include BO parameters only if productType == 'BO'
            if product_type == 'BO':
        ...
        try:
            ...
            for leg in legs:
                if leg['status'] not in ('EXECUTED', 'TRADED', 'CANCELLED', 'REJECTED'):
                    logger.info(f"Cancelling BO leg: {leg['leg_order_id']} (type: {leg['leg_type']})")  # ✅ PATCH-003: Fixed field name
                    
                    try:
                        self._wait_for_rate_limit()
                        self.follower_client.cancel_order(leg['leg_order_id'])  # ✅ PATCH-003: Fixed field name
                        self.db.update_bracket_order_leg_status(leg['id'], 'CANCELLED')
                    except Exception as e:
                        logger.error(f"Failed to cancel BO leg {leg['leg_order_id']}: {e}")  # ✅ PATCH-003: Fixed field name
        ...
        try:
            ...
            for leg in legs:
                if leg['leg_type'] == opposite_leg_type and leg['status'] not in ('EXECUTED', 'TRADED', 'CANCELLED', 'REJECTED'):
                    logger.info(f"OCO: Cancelling {opposite_leg_type} leg {leg['leg_order_id']}")  # ✅ PATCH-003: Fixed field name
