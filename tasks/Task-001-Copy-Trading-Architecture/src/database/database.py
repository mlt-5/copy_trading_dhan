...
class DatabaseManager:
    ...
    def save_bracket_order_leg(self, leg_data: Dict[str, Any]) -> bool:
        """
        ✅ TASK-006: Save bracket order leg to tracking table.
        
        Args:
            leg_data: Dictionary with parent_order_id, leg_type, order_id, status, etc.
        """
        try:
            self.conn.execute('''
    ...
    def get_bracket_order_legs(self, parent_order_id: str) -> List[Dict[str, Any]]:
        """
        ✅ TASK-006: Retrieve all legs for a bracket order.
